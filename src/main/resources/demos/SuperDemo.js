...
let PlayAnnimation = false,
  PlayerCharger = false,
  skybox,
  scene, sceneCharger = false,
  meshPlayer, meshOctree,
  skeletonsPlayer = [],
  cameraArcRotative = [], octree,
  keys= { letft:0,right:0,arriere:0,avancer:0},
  VitessePerso = 5,
  forward,
  backwards;
...
function createScene() {
  ...
  // Terrain
  let ground = BABYLON.Mesh.CreateGroundFromHeightMap("ground", "map.jpg", 100, 100, 100, 0, 12, scene, true);
  ...
  BABYLON.SceneLoader.ImportMesh("", "./", "him.babylon", scene, function (newMeshes, particleSystems, skeletons)
  {
    ...
    meshPlayer.position = new BABYLON.Vector3(-5.168, 5, -7.463);
...
function onKeyDown(event)
{
  ...
  let ch = String.fromCharCode(touche);
  switch (touche) {
